...
void RealSenseNodeFactory::startDevice()
{
    ...
    if (device_name == "RealSense D555" ||
        device_name == "Intel RealSense D555")
    {
    ...
    try
    {
      bool is_supported_dds_device = false;

      if (_device.supports(RS2_CAMERA_INFO_CONNECTION_TYPE) &&
        std::string(_device.get_info(RS2_CAMERA_INFO_CONNECTION_TYPE)) == "DDS")
      {
        const std::string product_line =
            _device.supports(RS2_CAMERA_INFO_PRODUCT_LINE)
                ? _device.get_info(RS2_CAMERA_INFO_PRODUCT_LINE)
                : "";

         const std::string name =
             _device.supports(RS2_CAMERA_INFO_NAME)
                 ? _device.get_info(RS2_CAMERA_INFO_NAME)
                 : "";

         is_supported_dds_device =
             (product_line == "D500") &&
             (name.find("D555") != std::string::npos);
      }

      if (is_supported_dds_device)
      {
        _realSenseNode =
            std::make_unique<BaseRealSenseNode>(
                *this,
                _device,
                _parameters,
       ...
       else
      {
        switch (pid)
        {
          ...
          default:
              ROS_FATAL_STREAM(
                  "Unsupported device! Product ID: 0x" << pid_str);
