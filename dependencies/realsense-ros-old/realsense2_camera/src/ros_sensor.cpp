...
void RosSensor::set_sensor_auto_exposure_roi()
{
    ...
    catch(const std::runtime_error& e)
    {
        ...
        {
            ROS_DEBUG_STREAM("Skipping unsupported auto exposure ROI setup for sensor "
                             << rs2_to_ros(get_info(RS2_CAMERA_INFO_NAME)) << ": "
                             << error_message);
            return;
        }
        ROS_ERROR_STREAM(__FILE__ << ":" << __LINE__ << ":" << error_message);
    }
}
