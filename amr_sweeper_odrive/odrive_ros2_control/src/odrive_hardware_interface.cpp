...
namespace amr_sweeper_odrive_ros2_control {
...
using namespace amr_sweeper_odrive_ros2_control;
...
return_type ODriveHardwareInterface::write(const rclcpp::Time&, const rclcpp::Duration&) {
    for (auto& axis : axes_) {
        ...
        if (axis.pos_input_enabled_) {
            ...
            msg.Input_Torque = axis.direction_sign_ * axis.torque_setpoint_;
            axis.send(msg);
        } else {
    ...
    switch (cmd) {
        ...
        case Get_Torques_msg_t::cmd_id: {
            if (Get_Torques_msg_t msg; try_decode(msg)) {
                torque_target_ = direction_sign_ * msg.Torque_Target;
                torque_estimate_ = direction_sign_ * msg.Torque_Estimate;
            }
        } break;
...
PLUGINLIB_EXPORT_CLASS(amr_sweeper_odrive_ros2_control::ODriveHardwareInterface, hardware_interface::SystemInterface)
