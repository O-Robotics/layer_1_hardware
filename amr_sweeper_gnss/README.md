...
```
cd ~/ros2_ws/src
git clone https://github.com/O-Robotics/amr_sweeper_gnss.git
```
...
```
git clone https://github.com/tilk/amr_sweeper_gnss.git
```

After install the package, build again in your workspcace:
```
colcon build --packages-select amr_sweeper_gnss
```
...
```
colcon build --packages-select amr_sweeper_gnss amr_sweeper_gnss amr_sweeper_gnss amr_sweeper_gnss amr_sweeper_gnss
```
...
```
ros2 param get /amr_sweeper_gnss CFG_MSGOUT_UBX_NAV_PVT_USB
```
```
ros2 param set /amr_sweeper_gnss CFG_MSGOUT_UBX_NAV_PVT_USB 1
```
## Ros2 echo NavSatFix
...
RTCM messages can be delivered externally. Alternately the amr_sweeper_gnss can be utilised to retrieve RTCM messages from a castor to publish `/ntrip_client/rtcm` messages which will be received by the amr_sweeper_gnss.
...
```
ros2 run amr_sweeper_gnss amr_sweeper_gnss --ros-args -p CFG_USBOUTPROT_NMEA:=False
```
Parameters can be set at launch and some examples are provided to launch high precision pos ecef|llh
```
ros2 launch amr_sweeper_gnss ublox_rover_hpposecef.launch.py
```
```
ros2 launch amr_sweeper_gnss ublox_rover_hpposllh.launch.py
```
...
```
ros2 service call /amr_sweeper_gnss/reset_odo amr_sweeper_gnss/srv/ResetODO
```
```
ros2 service call /amr_sweeper_gnss/cold_start amr_sweeper_gnss/srv/ColdStart '{reset_type: 1}'
```
```
ros2 service call /amr_sweeper_gnss/warm_start amr_sweeper_gnss/srv/WarmStart '{reset_type: 1}'
```
```
ros2 service call /amr_sweeper_gnss/hot_start amr_sweeper_gnss/srv/HotStart '{reset_type: 1}'
```
...
```
ros2 param set /amr_sweeper_gnss CFG_RATE_NAV 2
```
or the current value retrieved
```
ros2 param get /amr_sweeper_gnss CFG_RATE_NAV
```
### UBX Parameters

The following parameters may be set. Its a subset of the full UBX Gen 9 configuration parameter list.

Values will be as described in the integration manual (without scaling applied). Changing a CFG_MSGOUT_* type parameter to a value >0 will cause the corresponding amr_sweeper_gnss to be published.
...
By default, the amr_sweeper_gnss node will search for and connect to the first device which matches the ublox USB ID's (vendor ID of 0x1546 and product ID of 0x01a9).  If multiple devices are connected simultaneously, the remaining devices will be ignored.  In this situation you have no control over which device is used, since the order in which they are found may depend on the order in which they were physically attached to the host.
...
A basic NTRIP client is provided, that focuses in binary RTCM data. Its purpose is to acquire the RTCM data from an internet based NTRIP Castor and publish [RTCM Messages](https://index.ros.org/p/amr_sweeper_gnss/).

An example launch file is provided. You will need to set values appropriate for the NTRIP Castor intended to be used
```
ros2 launch amr_sweeper_gnss ntrip_client.launch.py use_https:=true host:=ntrip.data.gnss.ga.gov.au port:=443 mountpoint:=MBCH00AUS0 username:=username password:=password
```
...
After making sure that the deivce is processing RTCM messages and assuming that you have launched the node with `ros2 launch amr_sweeper_gnss ublox_rover_hpposllh.launch.py`, run the following the command
