...
- When native quaternion packets are enabled and present, the hardware serial path now also refreshes its cached Euler angles from that same quaternion sample before publishing. This keeps the hardware `data_raw` orientation derivation aligned with the simulation path, where Euler angles are already derived from the incoming quaternion before publish.
- On hardware, the serial parser now waits until it has a fresh accel packet, gyro packet, and the orientation packet family needed by the configured outputs before publishing a sample. This avoids publishing mixed-time stitched samples from whichever packet happened to arrive last, and makes the hardware publish cadence behave more like the single-message simulation callback.
